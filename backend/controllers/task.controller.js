...
const createTask = async (req, res) => {
    try {
        const { title, description, startDate, endDate, isCompleted, userId } = req.body;
        ...
        const newTask = await Task.create({ 
            userId, title, description, startDate, endDate, isCompleted 
        });
        res.status(201).json({ 
...
const updateTask = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: TASK_UPDATE_SUCCESS_MSG, updated });
    } catch (error) {
        res.status(500).json({ message: SERVER_ERROR_MSG });
    }
};
